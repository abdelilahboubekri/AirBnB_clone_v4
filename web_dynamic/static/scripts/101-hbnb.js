$(document).ready(function () {
  ...
  // Function to handle the click event on the "show" span
  $('.show-span').click(function () {
    const isHide = $(this).text().trim().toLowerCase() === 'hide';

    // Toggle the text between 'show' and 'hide'
    $(this).text(isHide ? 'show' : 'hide');

    if (isHide) {
      // If the text is 'hide', remove all Review elements from the DOM
      removeReviews();
    } else {
      // If the text is 'show', fetch and display reviews
